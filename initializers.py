...
class RandomUniformInitializer(Initializer):
  ...
  def __call__(self, shape):
    ...
    return random_state.uniform(
        low=self._params['minval'], high=self._params['maxval'], size=shape).astype(self._dtype)
...
class GlorotUniformInitializer(VarianceBasedInitializer):
  ...
  def __call__(self, shape):
    ...
    random_state = self._get_random_state()
    return random_state.uniform(low=-limit, high=limit, size=shape).astype(self._dtype)
...
class GlorotNormalInitializer(VarianceBasedInitializer):
  ...
  def __call__(self, shape):
    ...
    random_state = self._get_random_state()
    return random_state.normal(loc=0, scale=limit, size=shape).astype(self._dtype)
