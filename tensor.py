...
class Tensor(object):
  ...
  `tensor_index`). Tensors are considered "immutable" in the sense that their
  value can only be computed by their parent Op, and hense cannot be arbitrarily
  modified.

  Each tensor has a `shape` that indicates the shape of the multi-dimensional
  array at graph construction time. See `TensorShape` for details.
  """
